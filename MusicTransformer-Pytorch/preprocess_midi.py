...
def prep_custom_midi(custom_midi_root, output_dir, valid_p = 0.1, test_p = 0.2):
    ...
    for piece in (os.path.join(a, filename) for a,b,c in os.walk(custom_midi_root) for filename in c):
        ...
        mid         = os.path.join(piece)
        f_name      = "-".join(mid.split("/")[-2:]) + ".pickle"
        if not mid.endswith("midi"):
            continue
        if(split_type == "train"):
            o_file = os.path.join(train_dir, f_name)
            train_count += 1
